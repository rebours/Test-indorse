...
export class AppComponent {
   ...
   saveFood = this.junkFood;
   details: any;

   constructor() {
   }
   researchFood(event: any) {
    this.junkFood = this.saveFood;
    let foodResearch = [];
  ...
  detailsModal(name) {
    ...
    this.junkFood.forEach(function (item) {
      if (item.name === name) {
        foodResearch.push(item);
      }
    });
    this.details = foodResearch[0];
  }
}
